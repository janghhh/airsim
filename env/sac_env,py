...
class AirSimDroneEnv(AirSimEnv):
    ...
    def _compute_reward(self):
        ...
        # --- 1단계: 생존 확인 (가장 중요) ---
        if self.state["collision"] or min_dist_to_obstacle < 2.0:
            reward = -100.0
            done = 1
            return reward, done
    ...
    def visualize(self):
        ...
        for i in range(37):
            angle = (i / 36) * 2 * np.pi
            x = follower_pos.x_val + follower_radius * np.cos(angle)
            y = follower_pos.y_val + follower_radius * np.sin(angle)
            z = follower_pos.z_val
            follower_points.append(airsim.Vector3r(x, y, z))

        self.drone.simPlotLineStrip(
            points=follower_points,
            color_rgba=[0.0, 1.0, 0.0, 0.8],  # 초록색
            thickness=10.0,
            duration=0.1,
            is_persistent=False
        )
